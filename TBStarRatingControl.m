...
@implementation TBStarRatingControl
...
- (void)setupView {
	...
	for (int i=0; i<numberOfStars; i++) {
		TBStarView *v = [[TBStarView alloc] initWithDefault:self.star highlighted:self.highlightedStar position:i];
		[self addSubview:v];
	}
}
...
- (void)layoutSubviews {
	for (int i=0; i < numberOfStars; i++) {
		[(TBStarView*)[self subViewWithTag:i] centerIn:self.frame with:numberOfStars];
	}
}
